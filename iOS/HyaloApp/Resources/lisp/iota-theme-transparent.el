...
(defcustom iota-theme-transparent-in-gui t
  "Automatically remove background colors in graphical Emacs.
When enabled, backgrounds are replaced with the default face background
to achieve visual transparency while maintaining proper rendering."
  :type 'boolean
  :group 'iota-theme-transparent)
...
(defun iota-theme-transparent-should-apply-p ()
  ...
  (let ((should-apply (or (and iota-theme-transparent-in-terminal
                               (not (display-graphic-p)))
                          (and iota-theme-transparent-in-gui
                               (display-graphic-p)))))
    (iota-theme-transparent--log "Should apply transparency? %s (in-terminal: %s, in-gui: %s, graphic: %s)"
                                 should-apply
                                 iota-theme-transparent-in-terminal
                                 iota-theme-transparent-in-gui
                                 (display-graphic-p))
    should-apply))
...
Only modifies theme specs - does not set user specs."
  ...
  (if (not (iota-theme-transparent-should-apply-p))
    ...
    (let ((faces-modified 0)
          ...
          ;; In GUI mode, use default background; in terminal, use 'unspecified
          (replacement-bg (if (display-graphic-p)
                                       ...
                                       (mapcar
                                        (lambda (display-spec)
                                          ...
                                          (let* ((display (car display-spec))
                                                 ...
                                                 (new-attrs '()))
                                            ;; Process attributes, replacing :background and removing :underline, :overline, :box, :border
                                            (let ((bg-added nil))
                                              (while attrs
                                                (let ((key (car attrs))
                                                      (val (cadr attrs)))
                                                  (cond
                                                   ;; For :background, replace with replacement-bg in GUI or remove in terminal
                                                   ((eq key :background)
                                                    (when (display-graphic-p)
                                                      (push :background new-attrs)
                                                      (push replacement-bg new-attrs)
                                                      (setq bg-added t)))
                                                   ;; Remove these attributes entirely
                                                   ((memq key '(:underline :overline :box :border))
                                                    nil)
                                              ...
                                              ;; In GUI mode, ensure background is set even if not originally present
                                              (when (and (display-graphic-p) (not bg-added))
                                                (push :background new-attrs)
                                                (push replacement-bg new-attrs)))
                      ...
                      (if (memq face iota-theme-transparent-critical-faces)
                          ...
                          (progn
                            (when (memq face '(vertical-border window-divider window-divider-first-pixel
                                              window-divider-last-pixel fringe internal-border default
                                              iota-box-face iota-inactive-box-face))
                              (iota-theme-transparent--log "    %s: CRITICAL face, setting background to %s" face replacement-bg))
                            ;; Use set-face-attribute to set background and remove box
                            (set-face-attribute face nil :background replacement-bg :box nil)
                            (setq faces-modified (1+ faces-modified)))
