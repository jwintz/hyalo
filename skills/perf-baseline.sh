#!/bin/bash
# perf-baseline.sh — Record a 60s performance baseline for Hyalo (Emacs + libHyalo.dylib)
#
# Usage:
#   ./skills/perf-baseline.sh [--compare FILE_A FILE_B]
#   ./skills/perf-baseline.sh [--pid PID]
#
# If PID is omitted, attaches to the first running Emacs process.
# Saves results to skills/perf-results-YYYYMMDD-HHMMSS.txt (TSV format)
#
...
# -- Resolve PID (Emacs binary may be named Emacs or Emacs-arm64) -----------
...
if [[ -n "$PID_ARG" ]]; then
    ...
    if [[ -z "$PID" ]]; then
        ...
        echo "  Launch Emacs with libHyalo.dylib loaded, then re-run."
...
echo "=== Hyalo ($APP_NAME) Performance Baseline ==="
echo "PID: $PID"
echo "Duration: ${DURATION}s"
echo ""

# -- Hyalo-specific: check dylib presence -----------------------------------

DYLIB_LOADED=$(vmmap "$PID" 2>/dev/null | grep -c "libHyalo" || echo "0")
echo "Hyalo dylib regions: $DYLIB_LOADED"
...
# -- Emacs GC stats (before) ------------------------------------------------

EMACS_GC_BEFORE=""
EMACS_GC_AFTER=""
if command -v emacsclient &>/dev/null; then
    EMACS_GC_BEFORE=$(emacsclient --eval "(format \"%d\" gcs-done)" 2>/dev/null || echo "n/a")
fi
...
if command -v emacsclient &>/dev/null; then
    EMACS_GC_AFTER=$(emacsclient --eval "(format \"%d\" gcs-done)" 2>/dev/null || echo "n/a")
fi
...
hyalo_dylib_regions	${DYLIB_LOADED}
emacs_gc_before	${EMACS_GC_BEFORE:-n/a}
emacs_gc_after	${EMACS_GC_AFTER:-n/a}
pid	${PID}
process	${APP_NAME}
duration_s	${DURATION}
