...
- Fix Emacs face rendering on iPadOS: all faces rendered with `bg=0x0` (black) because `default-frame-alist` in `init-appearance.el` set `(alpha-background . 0.0)` unconditionally. On macOS this enables Liquid Glass pass-through; on iOS it causes xfaces.c face realization to encode transparent (0x0) background pixels, making every face background black. Guard `alpha-background` and `ns-alpha-elements` behind `(when (eq window-system 'ns) ...)`. Also add defensive `f->alpha_background = 1.0` in `ios/iosfns.m` frame creation to clamp any future Lisp-set transparency to opaque on iOS. Faces now realize with correct nano-dark theme colors; the dark background is correct — iOS falls back to `'dark` appearance since `ns-system-appearance` is NS-only and the channel bridge is not yet wired.
